...
export function activate(context: vscode.ExtensionContext) {
	context.subscriptions.push(vscode.commands.registerCommand('extension.runCMD', runTerminalCommand));
}
...
async function runTerminalCommand(uri: vscode.Uri | undefined) {
	...
	runCommand(pickedCommand, (await env).cwd, (await env).resource);
}
